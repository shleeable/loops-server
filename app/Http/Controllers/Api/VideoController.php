...
class VideoController extends Controller
{
    ...
    public function update(UpdateVideoRequest $request, $id)
    {
        ...
        $video->can_download = $request->has('can_download') ? $request->boolean('can_download') : $video->can_download;
        $video->can_embed = $canEmbed && $request->has('can_embed') ? $request->boolean('can_embed') : $video->can_embed;
        $video->comment_state = $request->has('can_comment') ? ($request->boolean('can_comment') ? 4 : 0) : $video->comment_state;
        $video->is_pinned = $request->has('is_pinned') ? $request->boolean('is_pinned') : $video->is_pinned;
